const contentEdital = document.getElementById('contentEdital');
const paginate = document.getElementById('paginate');
const loader = document.getElementById('loader');
const token = document.querySelector('[data-csrf]');
const route = document.querySelector('[data-search-edital-url]');
...
function search(element) {
    ...
    timer = setTimeout((name) => {
        ...
        .then((payload) => {
            payload.editals.length <= 0
                ? contentEdital.innerHTML = notFound()
                : contentEdital.innerHTML = getContent(payload.editals);
            loading(false);
        })
    ...
    return editals.reduce((accumulator, edital) =>
        accumulator + 
            `<tr class="text-dark">
                ...
                <td>
                    ${edital.status.value === 1
                        ?  `<a href="${edital.route.register}"
                    ...
                    <a href="${edital.route.edit}" title="Editar" class="btn btn-sm waves-effect mb-2 mb-md-0 px-1">
                        ...
                        <div class="dropdown-menu">
                            ...
                            <li class="dropdown-item">
                                <button type="button" data-toggle="modal" onclick="destroyEdital(this.dataset)"
                                    data-route="${edital.route.destroy}"
...
function destroyCustomer(payload) {
...
const loading = (isTrue) => {
    if (isTrue) {
        contentEdital.classList.add('d-none');
        paginate.classList.add('d-none');
        loader.classList.remove('d-none');
    } else {
        loader.classList.add('d-none');
        contentEdital.classList.remove('d-none');
