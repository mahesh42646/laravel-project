...
use App\Models\Project\IdentificationProject\Justification\Timeline;
...
final class IdentificationProjectJustificationController
{
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REPPROVED, 'justification');

            if ($project->identification_project_justification) {
                $project->identification_project_justification()->update([
                ...
                $project->identification_project_justification->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_justification->timelines
                );
            } else {
                $cronogram = $project->identification_project_justification()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_justification_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('justificativa do projeto'),
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REANALYZE, 'justification');
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::APPROVED, 'justification');
