...
use App\Models\Project\IdentificationProject\Target\Timeline;
...
final class IdentificationProjectTargetController
{
    ...
    public function repproved(Project $project, RepprovedRequest $request): JsonResponse
    {
        DB::beginTransaction();
            ...
            if ($project->identification_project_target) {
                $project->identification_project_target()->update([
                ...
                $project->identification_project_target->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_target->timelines
                );
            } else {
                $cronogram = $project->identification_project_target()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_target_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('público alvo do projeto'),
