...
use App\Models\Project\IdentificationProject\ContrapartidSocial\Timeline;
...
final class IdentificationProjectContrapartidSocialController
{
    ...
    public function repproved(Project $project, RepprovedRequest $request): JsonResponse
    {
        DB::beginTransaction();
            ...
            if ($project->identification_project_contrapartid_social) {
                $project->identification_project_contrapartid_social()->update([
                ...
                $project->identification_project_contrapartid_social->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_contrapartid_social->timelines
                );
            } else {
                $cronogram = $project->identification_project_contrapartid_social()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_contrapartid_social_id', $cronogram->id)->get()
                );
            }
