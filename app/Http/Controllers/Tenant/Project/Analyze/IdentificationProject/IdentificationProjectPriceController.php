...
final class IdentificationProjectPriceController
{
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REPPROVED, 'price');

            if ($project->identification_project_price) {
                $project->identification_project_price()->update([
                ...
                $project->identification_project_price->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_price->timelines
                );
            } else {
                $cronogram = $project->identification_project_price()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_price_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('valor do projeto'),
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REANALYZE, 'price');
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::APPROVED, 'price');
