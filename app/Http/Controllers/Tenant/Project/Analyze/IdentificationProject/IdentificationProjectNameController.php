...
use App\Models\Project\IdentificationProject\Name\Timeline;
...
final class IdentificationProjectNameController
{
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REPPROVED, 'name');

            if ($project->identification_project_name) {
                $project->identification_project_name()->update([
                ...
                $project->identification_project_name->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_name->timelines
                );
            } else {
                $cronogram = $project->identification_project_name()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_name_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('nome do projeto'),
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REANALYZE, 'name');
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::APPROVED, 'name');
