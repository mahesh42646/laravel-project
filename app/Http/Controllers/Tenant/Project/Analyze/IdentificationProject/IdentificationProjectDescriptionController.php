...
use App\Models\Project\IdentificationProject\Description\Timeline;
...
final class IdentificationProjectDescriptionController
{
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REPPROVED, 'description');

            if ($project->identification_project_description) {
                $project->identification_project_description()->update([
                ...
                $project->identification_project_description->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_description->timelines
                );
            } else {
                $cronogram = $project->identification_project_description()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_desccription_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('descrição do projeto'),
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REANALYZE, 'description');
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::APPROVED, 'description');
