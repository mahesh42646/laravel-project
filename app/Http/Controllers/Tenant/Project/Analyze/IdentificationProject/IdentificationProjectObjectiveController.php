...
use App\Models\Project\IdentificationProject\Objective\Timeline;
...
final class IdentificationProjectObjectiveController
{
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REPPROVED, 'target');

            if ($project->identification_project_objective) {
                $project->identification_project_objective()->update([
                ...
                $project->identification_project_objective->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_objective->timelines
                );
            } else {
                $cronogram = $project->identification_project_objective()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_objetive_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('objetivos e metas do projeto'),
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REANALYZE, 'target');
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::APPROVED, 'target');
            ...
            if ($project->identification_project_objective) {
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_objective_id', $cronogram->id)->get()
                );
            }
