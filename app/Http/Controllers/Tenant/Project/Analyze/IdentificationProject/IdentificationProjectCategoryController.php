...
use App\Models\Project\IdentificationProject\Category\Timeline;
...
final class IdentificationProjectCategoryController
{
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REPPROVED, 'cateogry');

            if ($project->identification_project_category) {
                $project->identification_project_category()->update([
                ...
                $project->identification_project_category->timelines()->create([
                ...
                $content = $this->service->getTimeline(
                    $project->identification_project_category->timelines
                );
            } else {
                $cronogram = $project->identification_project_category()->create([
                    'status' => AnalyzeStatusEnum::REPPROVED,
                    'analyzed_by' => Auth::id(),
                    'motive' => $request->motive,
                    'expired_at' => null,
                ...
                $content = $this->service->getTimeline(
                    Timeline::where('identification_project_category_id', $cronogram->id)->get()
                );
            }
        ...
        return response()->json([
            'message' => $request->message('categoria'),
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::REANALYZE, 'cateogry');
    ...
    {
        DB::beginTransaction();
            $this->notification->toAgent($project, TypeStatus::APPROVED, 'cateogry');
