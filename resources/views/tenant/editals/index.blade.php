<x-layout.panel>
    ...
    <input type="hidden" data-search-edital-url value="{{ route('editals.search') }}">

    {{-- HEADER --}}
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="mb-0">Lista de Editais</h3>
        <a href="{{ route('editals.create') }}" class="btn btn-primary waves-effect d-flex align-items-center">
            <svg xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 -960 960 960" width="22px" fill="#FFF">
                <path d="M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z"/>
            </svg>
            <span class="ml-1">Novo edital</span>
        </a>
    </div>
    ...
    <div class="card col-12 px-0">
        ...
        <div class="d-md-flex justify-content-between align-items-center px-2 pt-3">
            <div class="col-md-7 pr-0 d-flex mb-4">
                <div class="mr-3">
                    <a href="{{ route('editals.index') }}">
                        <div>
                            ...
                            <span class="btn-light font-weight-medium rounded-lg px-3 py-1">
                                {{ number_format($total->active + $total->pending + $total->inactive + $total->finished, 0, ',', '.') }}
                            </span>
                        </div>
                ...
                <div class="mr-3">
                    <a href="{{ route('editals.index') }}?status=1">
                        <div>
                ...
                <div class="mr-3">
                    <a href="{{ route('editals.index') }}?status=2">
                        <div>
                            <span class="font-weight-semibold text-{{ request('status') == '2' ? 'primary' : 'secondary' }} mr-1">
                                Pendentes
                            </span>
                            <span class="btn-light font-weight-medium rounded-lg px-3 py-1">
                                {{ number_format($total->pending, 0, ',', '.') }}
                            </span>
                        </div>
                        @if (request('status') == '2')
                            <div><hr class="text-primary mb-0" style="border: 2px solid; margin-top: 11px;"></div>
                ...
                <div class="mr-3">
                    <a href="{{ route('editals.index') }}?status=3">
                        <div>
                            <span class="font-weight-semibold text-{{ request('status') == '3' ? 'primary' : 'secondary' }} mr-1">
                                Inativos
                            </span>
                            <span class="btn-light font-weight-medium rounded-lg px-3 py-1">
                ...
                <div class="mr-3">
                    <a href="{{ route('editals.index') }}?status=4">
                        <div>
                            <span class="font-weight-semibold text-{{ request('status') == '4' ? 'primary' : 'secondary' }} mr-1">
                                Encerrados
                            </span>
                            <span class="btn-light font-weight-medium rounded-lg px-3 py-1">
                                {{ number_format($total->finished, 0, ',', '.') }}
                            </span>
                        </div>
                        @if (request('status') == '4')
                            <div><hr class="text-primary mb-0" style="border: 2px solid; margin-top: 11px;"></div>
            ...
            <div class="col-md-5 input-group mb-4">
                <input type="search" class="form-control mr-3" onkeyup="search(this)" placeholder="Pesquisar edital por nome">
                <a href="{{ route('editals.index') }}" class="btn btn-light waves-effect">Limpar filtros</a>
            </div>
        </div>
            ...
            <div class="col-lg-12 table-responsive">
                <table class="table table-hover table-centered table-sm table-nowrap">
                    <thead class="text-secondary bg--light">
                        <tr>
                            <th class="pl-4 py-2" style="width: 50%;">Nome</th>
                            <th style="width: 15%;">Criado em</th>
                            <th style="width: 15%;">Encerrado em</th>
                            <th style="width: 10%;">Status</th>
                            <th style="width: 10%;">Ação</th>
                        </tr>
                    </thead>
                    <tbody class="text-dark" id="contentEdital">
                        @foreach ($editals as $edital)
                            <tr class="text-dark">
                                <td class="pl-4 py-2 d-flex flex-column align-items-start font-weight-medium">
                                    <div class="text-dark font-weight-bold">{{ Str::limit($edital->name, 85)  }}</div>
                                    <div class="text-secondary">Valor do projeto: {{ $edital->price }}</div>
                                </td>
                                <td class="text-dark">{{ $edital->created_at }}</td>
                                <td class="text-dark">
                                    @if ($edital->closed_at && ! $edital->extended_at)
                                        {{ $edital->closed_at }}
                                    @else
                                        <div class="text-dark" style="text-decoration: line-through;">{{ $edital->closed_at }}</div>
                                        <div class="text-danger">Prorrogado:</div>
                                        <div class="text-danger">{{ $edital->extended_at }}</div>
                                    @endif
                                </td>
                                <td>{!! $edital->status->icon !!}</td>
                                <td>
                                    @if ($edital->status->value === 1)
                                        <a href="{{ route('public.register.customers.show', base64_encode($edital->id)) }}" 
                                            title="Link para cadastro" class="btn btn-sm waves-effect mb-2 mb-md-0 px-1" target="_blank"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="#635bff" viewBox="0 0 256 256">
                                    ...
                                    @endif
                                    <a href="{{ route('editals.edit', $edital->id) }}" title="Editar" class="btn btn-sm waves-effect mb-2 mb-md-0 px-1">
                                    ...
                                    <div class="btn-group dropleft">
                                        ...
                                        <div class="dropdown-menu">
                                            <li class="dropdown-item">
                                                <a href="{{ route('editals.assign.avaliator.index', $edital->id) }}"
                                                ...
                                                >
                                                    ...
                                                    <span class="ml-2 font-size-13">Vincular avaliador</span>
                                                </a>
                                            </li>
                                                
                                            <li class="dropdown-item">
                                                <button type="button" data-toggle="modal" onclick="destroyEdital(this.dataset)"
                                                    data-route="{{ route('editals.destroy', $edital->id) }}"
                                                    data-name="{{ $edital->name }}" data-target="#deleteEdital"
    ...
    {{-- MODAL DE DELETAR EDITAL --}}
    <div class="modal fade" id="deleteEdital">
    ...
    <x-slot:scripts>
        <script src="{{ asset('js/tenant/edital/search.js') }}?version=1"></script>
    </x-slot:scripts>
