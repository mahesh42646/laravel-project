<div class="px-3 pb-3" data-category-container>
    <div class="bg-white mb-3">
        <select class="form-control" name="identification_category_id" @if ($isRequireds[0]) required @endif>
            <option value="">Selecione</option>
            @foreach ($categories as $category)
                <option value="{{ $category->id }}"
                    @selected($project->identification_category_id == $category->id)
                >
                    {{ $category->name }}
                </option>
            @endforeach
    ...
    <div data-category-alert></div>
    <button type="button" class="w-100 bg-white btn text-dark px-3 py-1 mb-4"
        style="border: 1px solid #ced4da;" onclick="saveCategory(this)"
    >
